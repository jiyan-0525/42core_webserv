...
it is asking the OS: Please create a TCP socket for me."
...
Now we can bind the socket to the address from result: ```bind(server_fd, result->ai_addr, result->ai_addrlen);```
```
...
VERY IMPORTANT: We have now 2 sockets the first one is the listening socket, the 2nd one was created at the function call accept() -> the OS created a new socket the client_socket
```
Client socket
